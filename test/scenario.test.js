...
test('11-step success scenario', async () => {
  const { db, clock, app } = createTestApp();

  createEmployee(db, { name: 'Admin', username: 'admin', rateCents: 0, isAdmin: true });
  const aId = createEmployee(db, { name: 'Worker A', username: 'a', rateCents: 2000 });
  const bId = createEmployee(db, { name: 'Worker B', username: 'b', rateCents: 2500 });
...
test('guards: double clock-in idempotent, second task blocked, employee redaction', async () => {
  ...
  createEmployee(db, { name: 'Admin', username: 'admin', isAdmin: true });
  const aId = createEmployee(db, { name: 'A', username: 'a', rateCents: 2000 });
  const admin = await login(request, app, 'admin');
  const workerA = await login(request, app, 'a');
